...
def register():
    ...
    try:
        ...
        logging.error(f"Failed to add consumer to Redis list for consumer id {consumer_id}. Ref: {uuid_ref}")
...
def unregister():
    ...
    try:
        ...
        uuid_ref = str(uuid.uuid4())
        logging.error(f"Failed to remove consumer from Redis list for consumer id {consumer_id}. Ref: {uuid_ref}")
        logging.exception(ex)
...
def check_membership():
    ...
    try:
        ...
        uuid_ref = str(uuid.uuid4())
        logging.error(f"Failed to check for membership in Redis list for consumer id {consumer_id}. Ref: {uuid_ref}")
        logging.exception(ex)

        error_message = f"Failed to check for membership! Use Ref for details: {uuid_ref}"
        response = jsonify({"error": error_message})
